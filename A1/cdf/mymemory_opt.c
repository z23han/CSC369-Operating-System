...
In terms of reducing fragmentation, best-fit algorithm seems to be better; while 
in terms of reducing running time, first-fit algorithm appears to be better, but it 
requires further testing. Thus in my opinion, a combination of them with certain 
rubrics can produce a better memory allocation algorithm. But I don't have enough 
time to combine the 2 algorithms together for the optimization. 
*/
